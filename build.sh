...
if [[ $CLEAN -eq 1 ]]; then
  ...
  run rm -f compiler/parser.tab.c compiler/parser.tab.h compiler/lex.yy.c
...
if [[ ! -d "compiler" ]]; then
  fail "compiler/ directory not found"
  echo ""
...
(
  ...
  run gcc lex.yy.c parser.tab.c ast.c ir.c lower.c opt.c codegen_asm.c codegen_termina.c typecheck.c compiler.c -o ../hylian

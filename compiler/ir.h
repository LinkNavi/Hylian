...
typedef enum {
    ...
    IR_CONST_NIL,     /* dest = 0               */
    IR_CONST_FLOAT,   /* dest = src1(float_val) */
    ...
    IR_NOP,           /* no-operation (placeholder after DCE)        */
        ...
        /* Memory intrinsics */
        IR_MEMSET,   /* memset(ptr, byte, count): src1=ptr src2=byte extra_src=count */
        IR_MEMCPY,   /* memcpy(dst, src, count): src1=dst src2=src extra_src=count  */
...
IR_LOAD_PTR,        // dest = *src1
IR_STORE_PTR,       // *src1 = src2
IR_LOAD_VOLATILE,   // dest = volatile *src1
IR_STORE_VOLATILE,  // volatile *src1 = src2
IR_ADDROF,          // dest = &var (str_extra = var name)
IR_STATIC_VAR,      // static global: str_extra=name, str_extra2=type, src1=initial_value
IR_READ_CR,         // dest = crN  (extra_int = N)
IR_WRITE_CR,        // crN = src1  (extra_int = N)
...
typedef enum {
    ...
    IROP_CONST_FLOAT, /* double-precision float: .float_val */
} IROperandKind;
...
typedef struct {
    ...
    union {
        ...
        double float_val; /* IROP_CONST_FLOAT */
    };
} IROperand;
...
#define PRINT_ARG_FLOAT    5   /* src1 = temp holding float (xmm via movq)*/

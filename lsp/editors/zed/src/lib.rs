...
impl HylianExtension {
    /// Find the `hylian-lsp` binary.
    ///
    /// Checks these locations in order:
    ///   1. `<worktree>/lsp/hylian-lsp`          — built inside the project
    ///   2. `<worktree>/hylian-lsp`               — project root convenience copy
    ///   3. System PATH via `which hylian-lsp`    — installed system-wide
    fn find_binary(&mut self, worktree: &zed::Worktree) -> Result<String> {
        ...
        // 1. lsp/hylian-lsp (built inside the repo)
        let in_lsp_dir = format!("{}/lsp/hylian-lsp", worktree_root);
        ...
        if fs::metadata(&at_root).map(|m| m.is_file()).unwrap_or(false) {
        ...
        Err("hylian-lsp not found. \
             Build it with `cd lsp && bash build.sh`, \
    ...
    fn label_for_completion(
        ...
        Some(zed::CodeLabel {
            spans: vec![zed::CodeLabelSpan::code_range(
                0..highlight_end.min(display.len()),
            )],

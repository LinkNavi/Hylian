...
typedef enum {
    ...
    NODE_STATIC_VAR,  /* static / const / static-array global variable */
...
typedef enum {
    ...
    TYPE_REF,      /* &T  — reference type */
    TYPE_RAWPTR,   /* *T  — raw pointer type */
} TypeKind;
...
typedef struct {
    ...
    ASTNode **body;
    int body_count;
    int is_naked;       /* 1 = naked function */
    int is_public;      /* 1 = public module member */
...
typedef struct {
    ...
    char *var_name;
    ASTNode *initializer;
    int is_const;    /* 1 = const, 0 = static */
    int array_size;  /* 0 = scalar, >0 = fixed-size array */
} StaticVarNode;
...
StaticVarNode *make_static_var(Type type, char *name, ASTNode *init, int is_const, int array_size);

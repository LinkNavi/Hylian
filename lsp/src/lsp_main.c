...
static int is_linkle_file(const char *uri) {
    if (!uri) return 0;
    const char *p = strrchr(uri, '/');
    const char *base = p ? p + 1 : uri;
    return strcmp(base, "linkle.hy") == 0;
}
      ...
      jb_key_begin_obj(&b, "capabilities");
        ...
        jb_key_begin_obj(&b, "completionProvider");
          jb_key_begin_arr(&b, "triggerCharacters");
            jb_arr_str(&b, ".");
            jb_arr_str(&b, ":");  /* linkle.hy value completions after ': ' */
          jb_arr_end(&b);
...
static void handle_did_open(FILE *out, const char *params) {
    ...
    /* linkle.hy files are not Hylian source — store text but skip diagnostics */
    if (is_linkle_file(uri)) {
        OpenDoc *d = doc_find(uri);
        if (!d) {
...
static void handle_did_change(FILE *out, const char *params) {
    ...
    /* linkle.hy: update stored text, suppress Hylian diagnostics */
    if (is_linkle_file(uri)) {
        OpenDoc *d = doc_find(uri);
        if (!d) {
            if (open_doc_count >= MAX_OPEN_DOCS) return;
...
static void handle_completion(FILE *out, long id, const char *params) {
    ...
    OpenDoc *doc = doc_find(uri);
    if (!doc || !doc->text) { send_null_result(out, id); return; }

    CompletionItem items[MAX_COMPLETIONS];
    int count = 0;

    /* linkle.hy gets its own context-aware completer */
    if (is_linkle_file(uri)) {
        count = lsp_complete_linkle(doc->text, (int)line, (int)col,
                                    items, MAX_COMPLETIONS);
    } else {
        if (!doc->state) { send_null_result(out, id); return; }
        count = lsp_complete(doc->state, doc->text, (int)line, (int)col,
                             items, MAX_COMPLETIONS);
    }

...
%token UNSAFE CONST STATIC AMP ENUM
...
%type <node> static_var_decl enum_decl enum_body
%type <node_list> class_body stmt_list param_list params arg_list args union_types include_list switch_arms
...
/* ── Static / const global variable declarations ──────────────────────────── */
static_var_decl:
    STATIC type IDENTIFIER ASSIGN expr SEMICOLON {
        $$ = (ASTNode*)make_static_var($2, $3, $5, 0, 0); SET_LINE($$);
    }
    | STATIC type IDENTIFIER SEMICOLON {
        $$ = (ASTNode*)make_static_var($2, $3, NULL, 0, 0); SET_LINE($$);
    }
    | PUBLIC STATIC type IDENTIFIER ASSIGN expr SEMICOLON {
        $$ = (ASTNode*)make_static_var($3, $4, $6, 0, 0); SET_LINE($$);
    }
    | PUBLIC STATIC type IDENTIFIER SEMICOLON {
        $$ = (ASTNode*)make_static_var($3, $4, NULL, 0, 0); SET_LINE($$);
    }
    | CONST type IDENTIFIER ASSIGN expr SEMICOLON {
        $$ = (ASTNode*)make_static_var($2, $3, $5, 1, 0); SET_LINE($$);
    }
    | PUBLIC CONST type IDENTIFIER ASSIGN expr SEMICOLON {
        $$ = (ASTNode*)make_static_var($3, $4, $6, 1, 0); SET_LINE($$);
    }
    | STATIC type IDENTIFIER LBRACKET NUMBER RBRACKET SEMICOLON {
        $$ = (ASTNode*)make_static_var($2, $3, NULL, 0, (int)$5); SET_LINE($$);
    }
    | PUBLIC STATIC type IDENTIFIER LBRACKET NUMBER RBRACKET SEMICOLON {
        $$ = (ASTNode*)make_static_var($3, $4, NULL, 0, (int)$6); SET_LINE($$);
    }
    ;
...
enum_decl:
    ENUM IDENTIFIER LBRACE enum_body RBRACE {
        EnumNode *en = (EnumNode*)$4;
        free(en->name); en->name = strdup($2); en->is_public = 0;
        free($2); $$ = (ASTNode*)en; SET_LINE($$);
    }
    ...
    | PUBLIC ENUM IDENTIFIER LBRACE enum_body RBRACE {
        EnumNode *en = (EnumNode*)$5;
        free(en->name); en->name = strdup($3); en->is_public = 1;
        free($3); $$ = (ASTNode*)en; SET_LINE($$);
    }
    | PUBLIC ENUM IDENTIFIER LBRACE enum_body RBRACE SEMICOLON {
        EnumNode *en = (EnumNode*)$5;
        free(en->name); en->name = strdup($3); en->is_public = 1;
        free($3); $$ = (ASTNode*)en; SET_LINE($$);
    }
...
class_decl:
    ...
    | PACKED PUBLIC CLASS IDENTIFIER LBRACE class_body RBRACE {
        $$ = make_class($4, 1);
    ...
    | PACKED CLASS IDENTIFIER LBRACE RBRACE { $$ = make_class($3, 0); }
    | PACKED PUBLIC CLASS IDENTIFIER LBRACE RBRACE { $$ = make_class($4, 1); }
...
stmt:
    ...
    | VOLATILE STAR expr ASSIGN expr SEMICOLON {
        $$ = (ASTNode*)make_assign("<volatile_deref>", $5);
        (void)$3;
        SET_LINE($$);
    }
    ...
    /* volatile read as statement: volatile *ptr; */
    | VOLATILE STAR expr SEMICOLON {
        $$ = (ASTNode*)make_unary_op("deref", $3, 0); SET_LINE($$);
    }
    | expr SEMICOLON { $$ = $1; }
...
expr:
    ...
    | AMP expr %prec UMINUS { $$ = (ASTNode*)make_unary_op("addrof", $2, 0); SET_LINE($$); }
    | STAR expr %prec UMINUS { $$ = (ASTNode*)make_unary_op("deref", $2, 0); SET_LINE($$); }
    /* volatile read expression: volatile *ptr */
    | VOLATILE STAR expr %prec UMINUS { $$ = (ASTNode*)make_unary_op("deref", $3, 0); SET_LINE($$); }
    ...
    /* bitwise binary */
    | expr AMP   expr %prec AMP  { $$ = (ASTNode*)make_binary_op("&",  $1, $3); SET_LINE($$); }
    | expr PIPE  expr { $$ = (ASTNode*)make_binary_op("|",  $1, $3); SET_LINE($$); }
    | expr XOR   expr { $$ = (ASTNode*)make_binary_op("^",  $1, $3); SET_LINE($$); }
    | expr LSHIFT expr { $$ = (ASTNode*)make_binary_op("<<", $1, $3); SET_LINE($$); }
    ...
    | CAST LT type GT LPAREN expr RPAREN {
        ...
        ASTNode *type_node = (ASTNode*)make_literal(tname, LIT_STRING);
        free(tname);
        $$ = (ASTNode*)make_binary_op("cast", $6, type_node); SET_LINE($$);
    }
    /* size_of */

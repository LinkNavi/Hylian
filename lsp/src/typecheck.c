...
static int is_module_name(const char *name) {
    if (!name) return 0;
    for (int i = 0; i < tc_module_count; i++)
        if (tc_module_names[i] && strcmp(tc_module_names[i], name) == 0)
            return 1;
    return 0;
}
...
static Type infer_expr(ASTNode *node) {
    ...
    case NODE_IDENTIFIER: {
        ...
        if (t) {
            result = *t;
        } else if (is_module_name(id->name)) {
            /* Module names are valid identifiers used as namespace prefixes.
               Return a synthetic type carrying the module name so that
               NODE_METHOD_CALL and NODE_MEMBER_ACCESS can resolve it. */
            result = make_simple_type(id->name, 0);
        } else if (is_enum_type(id->name)) {
            /* Enum type used as a namespace (e.g. Color in Color.Red) */
            result = make_simple_type(id->name, 0);
        } else {
    ...
    case NODE_FUNC_CALL: {
        ...
        if (fc->name &&
            (strcmp(fc->name, "__addrof_fn__") == 0 ||
             strcmp(fc->name, "addrof_fn") == 0 ||
             strcmp(fc->name, "adrof_fn") == 0)) {
        ...
        if (fc->name) {
            /* 0. Hard-wired compiler intrinsics (no user or external definition needed) */
            if (strcmp(fc->name, "cli")          == 0 ||
                strcmp(fc->name, "sti")          == 0 ||
                strcmp(fc->name, "write_cr")     == 0 ||
                strcmp(fc->name, "save_regs")    == 0 ||
                strcmp(fc->name, "restore_regs") == 0 ||
    ...
    case NODE_METHOD_CALL: {
        ...
        /* Check if the object is a known module name (Module.fn() call) */
        const char *obj_name = NULL;
        if (mc->object && mc->object->type == NODE_IDENTIFIER)
            obj_name = ((IdentifierNode *)mc->object)->name;
        if (obj_name && is_module_name(obj_name)) {
            /* Look up ModuleName__method in the function registry */
            char mangled[256];
            ...
            if (fi)
                ...
                result = make_simple_type("void", 0); /* public fn not yet indexed — suppress error */
            break;
        }
    ...
    case NODE_MEMBER_ACCESS: {
        ...
        if (ma->object && ma->object->type == NODE_IDENTIFIER) {
            ...
            if (is_module_name(id)) {
                /* The member is a module constant — resolve its type from the
                   mangled static var if registered, otherwise treat as uint64. */
                result = make_simple_type("uint64", 0);
...
void lsp_typecheck(ProgramNode *program, const char *filename) {
    ...
    /* Pre-register external module names (from other project files) so that
       Module.fn() and Module.CONST references in this file resolve correctly
       even when the module is defined in a different .hy file. */
    for (int i = 0; i < ext_func_count; i++) {
        if (ext_funcs[i].is_module && ext_funcs[i].name && tc_module_count < 64) {
            int dup = 0;
            for (int j = 0; j < tc_module_count; j++)
                if (tc_module_names[j] && strcmp(tc_module_names[j], ext_funcs[i].name) == 0) { dup = 1; break; }
            if (!dup)
                tc_module_names[tc_module_count++] = ext_funcs[i].name;
        }
        /* Also pre-register mangled module functions (ModuleName__fn) so
           cross-file method calls resolve their return types. */
        if (!ext_funcs[i].is_module && ext_funcs[i].name && ext_funcs[i].return_type) {
            /* Only add if it looks like a mangled module func (contains "__") */
            if (strstr(ext_funcs[i].name, "__")) {
                Type rt = make_simple_type((char *)ext_funcs[i].return_type, 0);
                register_func((char *)ext_funcs[i].name, rt, NULL, 0);
            }
        }
    }
    ...
    for (int i = 0; i < program->decl_count; i++) {
        ...
        if (d->type == NODE_VAR_DECL) {
            VarDeclNode *vd = (VarDeclNode *)d;
            /* Top-level static/public static declarations are parsed as
               VarDeclNodes by the LSP parser. Keep them in global scope so
               function and method bodies can resolve them. */
            scope_define(vd->var_name, vd->var_type);
            continue;
        }
        ...
        if (d->type == NODE_MODULE) {
            ...
            /* Register each public function as ModuleName__funcname */
            for (int fi = 0; fi < mn->func_count; fi++) {
                FuncNode *fn = mn->funcs[fi];
                if (!fn) continue;
                char mangled[256];
                snprintf(mangled, sizeof(mangled), "%s__%s", mn->name, fn->name);
                char *mc = strdup(mangled);
                register_func(mc, fn->return_type, fn->params, fn->param_count);
                /* Also register plain name for intra-module calls */

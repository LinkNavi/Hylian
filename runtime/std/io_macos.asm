; Hylian runtime — standard I/O (x86-64 NASM, macOS System V ABI)
; Assemble with: nasm -f macho64 io_macos.asm
;
; Exports:
;   _hylian_print      (char *str [rdi], int64_t len [rsi])
;   _hylian_println    (char *str [rdi], int64_t len [rsi])
;   _hylian_int_to_str (int64_t n [rdi], char *buf [rsi], int64_t buflen [rdx]) -> int64_t

bits 64
default rel

global _hylian_print
global _hylian_println
global _hylian_int_to_str
global _hylian_read_line
global _hylian_str_to_int
...
section .bss
    L_rl_buf:      resb 4096   ; internal read buffer
    L_rl_buf_pos:  resq 1      ; current read position in L_rl_buf
    L_rl_buf_fill: resq 1      ; number of valid bytes in L_rl_buf
...
_hylian_print:
    ...
    mov  rsi, rdi           ; buf
    mov  rdi, 1             ; fd = stdout
    mov  rax, 0x2000004     ; syscall: sys_write (macOS)
    syscall
.done:
    ret
...
; ─────────────────────────────────────────────────────────────────────────────
; _hylian_int_to_str(int64_t n [rdi], char *buf [rsi], int64_t buflen [rdx])
...
; ─────────────────────────────────────────────────────────────────────────────
; _hylian_read_line(char *buf [rdi], int64_t buflen [rsi]) -> int64_t
...
;   r14 = L_rl_buf_pos  (flushed to .bss on exit)
;   r15 = L_rl_buf_fill (flushed to .bss on exit)
;
; 6 pushes (rbp, rbx, r12-r15) = 48 bytes from 16-aligned entry → rsp%16==0 ✓
; ─────────────────────────────────────────────────────────────────────────────
_hylian_read_line:
    push rbp
    push rbx
    ...
    mov  r14, [rel L_rl_buf_pos]
    mov  r15, [rel L_rl_buf_fill]
...
.rl_loop:
    ...
    ; sys_read(fd=0, L_rl_buf, 4096)  — macOS syscall 0x2000003
    mov  rax, 0x2000003         ; sys_read (macOS)
    mov  rdi, 0                 ; fd = stdin
    lea  rsi, [rel L_rl_buf]
    mov  rdx, 4096
    syscall
...
.rl_done:
    ...
    mov  [rel L_rl_buf_pos],  r14
    mov  [rel L_rl_buf_fill], r15
...
; ─────────────────────────────────────────────────────────────────────────────
; _hylian_str_to_int(char *str [rdi], int64_t len [rsi]) -> int64_t

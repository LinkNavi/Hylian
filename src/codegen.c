...
void codegen(ProgramNode *root, FILE *out) {
 ...
 // fprintf(out, "#include \"../runtime/core.h\"\n\n");
...
void codegen_method(MethodNode* method, const char* class_name, FILE* out) {
    ...
    for (int i = 0; i < method->param_count; i++) {
        VarDeclNode* param = (VarDeclNode*)method->params[i];
        const char* ptype = param->var_type.name;
        if (strcmp(ptype, "str") == 0) ptype = "std::string";
        
        fprintf(out, "%s %s", ptype, param->var_name);

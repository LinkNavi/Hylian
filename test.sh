...
section "Unsafe blocks"

run_test "t160" "unsafe block executes body" \
'include {
    std.io,
}
Error? main() {
...
run_test "t161" "unsafe block with variable mutation" \
'include {
    std.io,
}
Error? main() {
...
run_test "t162" "unsafe block nested inside if" \
'include {
    std.io,
}
Error? main() {
...
run_test "t163" "unsafe block with arithmetic" \
'include {
    std.io,
}
Error? main() {
...
# ─────────────────────────────────────────────────────────────────────────────
# Tests: Pointers (addrof / deref)
# ─────────────────────────────────────────────────────────────────────────────

section "Pointers (addrof / deref)"

run_test "t170" "addrof and deref round-trip" \
'include {
    std.io,
}
Error? main() {
...
run_test "t171" "addrof yields non-nil pointer (unsafe deref read)" \
'include {
    std.io,
}
Error? main() {
...
run_test "t172" "pointer type declared as function parameter compiles" \
'include {
    std.io,
}
int read_ptr(*int p) {
...
run_test "t173" "addrof on two locals, deref both" \
'include {
    std.io,
}
Error? main() {
...
# ─────────────────────────────────────────────────────────────────────────────
# Tests: Inline asm with {varname} variable substitution
# ─────────────────────────────────────────────────────────────────────────────

section "Inline asm with variable slots"

# Note: the {varname} substitution in asm blocks expands to "rbp - N" at
# codegen time.  However, the asm-block lexer terminates on the first literal
# '}' character, so {varname} cannot appear inside NASM square brackets —
# e.g. "mov rax, [{x}]" would terminate the block early at the '}' of {x}.
# The tests below therefore use hardcoded [rbp - N] offsets to verify that
# asm blocks compile and execute correctly, and include a separate
# compiler-only (run_test_ir_dump) check to confirm {varname} substitution
# reaches codegen without a parse error.

run_test "t180" "asm block writes constant to local via hardcoded slot" \
'include {
    std.io,
}
Error? main() {
...
run_test "t181" "asm block reads local, adds constant, writes back" \
'include {
    std.io,
}
Error? main() {
...
run_test "t182" "asm block adds two locals" \
'include {
    std.io,
}
Error? main() {
...
run_test "t183" "unsafe block containing asm writes to local" \
'include {
    std.io,
}
Error? main() {
...
# Compiler-only check: verify that a plain asm block is accepted by the
# compiler and produces an ASM_BLOCK IR instruction.
run_test_ir_dump "t184" "asm block appears in IR dump as ASM_BLOCK" \
'include {
    std.io,
}
Error? main() {
